...
Two aliases make launching Claude Code a lot faster once you are using it daily:

- `clauded` — launches Claude Code with `--dangerously-skip-permissions` so you are not prompted to approve every file operation
- `jarvis` — cd into your Jarvis workspace and launch Claude Code in one command, from anywhere

**The cleanest way to set these up is to let Claude Code do it for you.**
...
> Please add two shell aliases to my shell config file (detect my shell and use `~/.zshrc`, `~/.bashrc`, or my PowerShell profile as appropriate). Preserve everything already in the file. The aliases:
>
> 1. `clauded` should run `claude --dangerously-skip-permissions`
> 2. `jarvis` should `cd` into `~/my-jarvis` and then launch `claude`
>
> After you add them, tell me what file you edited and remind me to open a new terminal window for the aliases to take effect.

Claude Code will detect your shell, find or create the right config file, and append the aliases without touching your existing setup. Open a new terminal window afterward and the aliases are live.
...
- `jarvis` anytime you want to jump straight into your Jarvis from a fresh terminal
