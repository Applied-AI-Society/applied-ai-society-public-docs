...
We maintain the Minimum Viable Jarvis starter as a **GitHub template repository**, so creating your own private copy is one click. Do not clone our repo directly. Do this instead:
...
4. Clone your new private repo and set up a safety-disabled upstream remote so you can pull template updates later without ever pushing back:
...
Future template updates can be pulled anytime by asking your Jarvis to run the `sync-with-upstream` skill that ships in the repo.
