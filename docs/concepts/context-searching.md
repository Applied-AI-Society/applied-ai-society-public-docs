...
*The runtime side of [context engineering](./context-engineering). How an agent actually pulls the right context out of your knowledge base at the moment it needs it.*
...
What happened: the agent is dumping too much into context, or pulling the wrong slice, or missing the obvious connection between two files. Output gets vague. Hallucinations creep back in. The practitioner blames the model.
...
Most modern agentic harnesses (Claude Code, Cursor, etc.) do this naturally if you let them. The agent has a search tool, it uses the tool, it reasons about results, it searches again. The practitioner's job is to give it good search tools and to structure the knowledge base so iterative search converges.
...
If you're helping someone build a Personal Agentic OS, context searching is where the real engineering happens. Anyone can set up a vault. The differentiator is whether the agent can find the right slice of that vault when it counts.
...
Context engineering builds the lake. Context searching is what makes the lake actually useful at runtime. Both are skills the practitioner has to develop.

---

## Further Reading

- [Context Engineering](./context-engineering): The supply side. Building the knowledge base in the first place.
- [Context Lake](./context-lake): The structured collection of files that context searching queries.
- [Compounding Docs](./compounding-docs): Why every file you add (and every link you add between files) makes every future search better.
- [PRM](./prm): Personal Relationship Management. The pattern of structuring person files as graph nodes that link out to everything connected to that person.
- [Hypercontext Protocol](./hypercontext-protocol): Where context searching goes when agents start querying each other's context lakes through trusted permission surfaces.
- [What Is Context Engineering? Connected Access, Knowledge Layer, Precision Retrieval, and Runtime Governance](https://www.youtube.com/watch?v=pN-LfxNFiTc) (Martin Keen, IBM): Frames the same retrieval taxonomy at the enterprise scale, with strong material on precision retrieval and the broader architecture context searching sits inside.
