...
A skill file is a structured prompt and instruction set that tells an [agentic harness](./agentic-harness) how to do a specific task: format inputs, run the right steps, call the right tools, return the right output. Read more in [Fat Skills](./fat-skills) and [Instruction Files](./instruction-files).
...
The first audience for any new capability is you. *[See: [Context Lake](./context-lake) and [Externalize Your Brain](./externalize-your-brain).]*
...
Once the skill file works for you, the next layer is people you trust who run their own [Personal Agentic OS](./personal-agentic-os) setups. They can run the skill file on their context the way you ran it on yours. The friction is low because they already have the tooling. The feedback is high because they will tell you the truth.

This is also the natural moment to evolve from a personal skill file to a community-distributed one. *[See: [Compounding Docs](./compounding-docs).]* The skill file becomes shareable. Another operator can drop it into their own context lake and run it.
...
For an [Applied AI Practitioner](./applied-ai-practitioner) who is building tools for clients, this order becomes the standard methodology.

- Diagnose the capability the client needs.
- Build it as a skill file that runs on the client's context lake.
