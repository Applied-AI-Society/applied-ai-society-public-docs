...
*A self-contained folder that wires your Jarvis up to take one specific action with one specific service. Markdown narrative plus a drop-in SKILL.md plus any helper scripts. One folder, one action, runnable.*

---

## What An Integration Recipe Is

An integration recipe is a unit of "wire your Jarvis up to do X." Where `X` is something concrete: create a Google Doc, send a Slack message, append a row to a spreadsheet, post to Notion, create a calendar event.
...
Drop the folder into your skill directory, run the setup commands, and your Jarvis can now do that one thing in the world.
...
The discipline: one recipe wires up one specific action. The folder name describes the action in [verb-action shape](/docs/concepts/skill-file-first-app-second), kebab-case, matching the SKILL.md `name:` field exactly.
...
1. **What This Gives You**: the capabilities your Jarvis gains.
...
5. **Usage From Your Jarvis**: what trigger phrases work.
...
|--------|----------|---------|
| **Integration recipe** (this concept) | Personal Jarvis operator | Wire up a specific action with a specific service |
| [SKILL.md](/docs/concepts/skill-file-first-app-second) (alone) | Your agent | Tell the agent how to perform a capability |
| [INTEGRATE.md](/docs/standards/integrate-md) (open spec) | A coding agent integrating a library into a target codebase | Teach the agent how to wire that library into an unknown project |
...
A recipe can live anywhere markdown lives, but the discipline is to publish recipes you trust into a public bank so other practitioners can fork them. The reference implementation: [garysheng/curated-jarvis-integrations](https://github.com/garysheng/curated-jarvis-integrations), an MIT-licensed bank with one folder per recipe.
...
The bank pattern matches [forkable-is-the-new-sticky](/docs/concepts/forkable-is-the-new-sticky): the recipes get more useful as more people fork them and contribute back, while the underlying mechanism (folder of files) stays trivially understandable.

## When To Reach For A Recipe

- You want your Jarvis to take a specific action in a specific service and the [universal copy-paste path](/docs/concepts/sharing-jarvis-artifacts) is no longer fast enough.
- You publish artifacts at enough volume that automating the publish step earns its keep.
- You want a teammate's Jarvis to do the same thing yours does, and you need to hand them a runnable artifact rather than a writeup.
...
When you find yourself doing the same wiring twice, write the recipe. When you find an existing recipe in a public bank that fits, fork it instead of writing your own. When you ship one, date it, name a maintainer, and list at least one alternative path. That is the contract. That is what makes recipes compound across the network of [activated practitioners](/docs/philosophy/jarvis-the-world-or-ai-is-doomed) instead of getting stale on individual machines.

---

## Further Reading

- [Sharing Jarvis Artifacts](/docs/concepts/sharing-jarvis-artifacts): The downstream practice that often motivates writing a recipe.
- [Skill File First, App Second](/docs/concepts/skill-file-first-app-second): Why SKILL.md is the unit a recipe wraps.
- [Forkable Is The New Sticky](/docs/concepts/forkable-is-the-new-sticky): Why bundling recipes for forking matters.
- [INTEGRATE.md spec](/docs/standards/integrate-md): The adjacent open standard for library authors teaching agents to wire their library into a target codebase. Different audience and shape; worth reading once to keep them straight.
- [Capture, Process, Compound](/docs/concepts/capture-process-compound): The general loop recipes accelerate.
